...
<head>
    ...
    <style type="text/css">
        ...
        .fs-level-chip { display:inline-block; font-size:10px; font-weight:700; letter-spacing:.5px;
            padding:2px 7px; border-radius:10px; margin-right:8px; vertical-align:middle; }
        .fs-chip-consol { background:#1d2c9e; color:#fff; }
        .fs-chip-pkg    { background:#cfd8e3; color:#2b3a4a; }

        /* Level 1 — consolidation card */
        .fs-consol-card   { border:1px solid #c7cfdb; border-left:5px solid #1d2c9e; border-radius:6px; overflow:hidden; }
        .fs-consol-header { background:#f4f6fb; color:#1f2d3d; cursor:pointer; font-size:15px; }
        .fs-consol-header b { color:#16245f; }
        .fs-consol-header .btn-light, .fs-consol-header .btn-light i { color:#212529 !important; }
        .fs-consol-header .fs-dim { opacity:.85; font-size:13px; }
        /* Active consolidation: tint the WHOLE card so its packages sit on this bg. */
        .fs-consol-card.fs-active { background:#e7ecfb; box-shadow: inset 0 0 0 2px #1d2c9e; }
        .fs-consol-card.fs-active > .fs-consol-header { background:#dbe3fb; }

        /* Level 2 — package card (sits on the consolidation's background) */
        .fs-pkg-card   { border:1px solid #dfe4ea; border-left:4px solid #8aa0bd; border-radius:5px; margin-left:12px; background:#fff; overflow:hidden; }
        .fs-pkg-header { background:#f7f9fb; color:#26333f; cursor:pointer; }
        .fs-pkg-header b, .fs-pkg-header i { color:#26333f; }
        /* Active package: a DIFFERENT bg so its items stand out from the consolidation. */
        .fs-pkg-card.fs-active { background:#fff7e6; box-shadow: inset 0 0 0 2px #e0a800; }
        .fs-pkg-card.fs-active > .fs-pkg-header { background:#ffeec6; }

        /* Level 3 — items */
        .fs-items-table { margin-left:6px; background:#fff; }
        .fs-items-table input:disabled { background:#f3f4f6; }

        /* Money */
        .fs-money { font-weight:700; color:#1b8a4b; white-space:nowrap; }

        /* Dangerous-goods marker: warning icon only, in a coloured circle. */
        .fs-dg-badge { display:inline-flex; align-items:center; justify-content:center;
        ...
        .fs-cur-mini .fs-cur-btn { padding:0 7px; font-size:12px; line-height:18px; height:20px; font-weight:700; }

        /* Package change log */
        .fs-history { margin-top:10px; padding-top:6px; border-top:1px solid #e3e6ea; font-size:12px; }
        .fs-history-title { font-weight:700; color:#5a6b7b; margin-bottom:3px; }
        .fs-hist-item { padding:2px 0; }
