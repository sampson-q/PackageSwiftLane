<aside class="left-sidebar">
	...
	<div class="scroll-sidebar">
		...
		<nav class="sidebar-nav">
			...
			<?php // One permission-gated menu for superadmin + staff + agency: every item
			      // below is gated by cdp_hasPermission for the exact permission its page
			      // enforces, so a user sees precisely what their role/department grants
			      // (superadmin passes all via the wildcard). Clients and drivers keep
			      // their own tailored menus below. ?>
			<?php if (cdp_roleHasFlag($userData->userlevel, 'is_superadmin') || cdp_roleHasFlag($userData->userlevel, 'is_staff') || cdp_roleHasFlag($userData->userlevel, 'is_agency')) { ?>
				...
				<ul id="sidebarnav">
					...
					<?php } ?>
                    <?php if ($user->cdp_hasPermission('warehouse_view') || $user->cdp_hasPermission('view_warehouse_delivery')) { ?>
                    <li class="sidebar-item nav-divider"></li>
					<!-- Module Warehouse -->
					<li class="sidebar-item">
						<a class="sidebar-link has-arrow waves-effect waves-dark" href="javascript:void(0)" aria-expanded="false">
					...
					<li class="sidebar-item">
						...
						<ul aria-expanded="false" class="collapse  first-level">
							...
							<?php if ($user->cdp_hasPermission('view_system_logs')) { ?>
							<li class="sidebar-item">
