<?php
// ============================================================================
// Consolidations Control Panel (Sea Container) — cdb_consolidate at a glance.
// Counts only: consolidation money lives on the Financial Sheet (per-customer
// billing), so this panel links there instead of re-deriving figures from the
// legacy total_order snapshot.
// ============================================================================
...
if ($user->cdp_hasPermission('view_dashboard_shipments')) {
    $ct_total     = cdp_dashCount('cdb_consolidate', "AND status_courier != 21" . $agency_where);
    $ct_open      = cdp_dashCount('cdb_consolidate', "AND status_courier NOT IN (8,21)" . $agency_where);
    $ct_transit   = cdp_dashCount('cdb_consolidate', "AND status_courier = 3" . $agency_where);
    $ct_delivered = cdp_dashCount('cdb_consolidate', "AND status_courier = 8" . $agency_where);
    $ct_cancel    = cdp_dashCount('cdb_consolidate', "AND status_courier = 21" . $agency_where);
    $ct_month     = cdp_dashCount('cdb_consolidate', "AND status_courier != 21 AND c_date >= '" . date('Y-m-01') . "'" . $agency_where);
...
<body>
        ...
        <div class="page-wrapper">
            <div class="page-breadcrumb">
                <div class="sw-dash-hello">
                    <div>
                        <h4 class="page-title mb-0"><?php echo $lang['left-menu-sidebar-23'] . ' (Sea Container)'?></h4>
                    ...
                    <div class="sw-quick-actions">
                        ...
                        <a href="consolidate_add.php" class="btn btn-sm btn-dark"><iconify-icon icon="solar:add-circle-linear"></iconify-icon> New Consolidation</a>
                        <?php } ?>
                        <a href="consolidate_list.php" class="btn btn-sm btn-outline-dark"><?php echo $lang['dash-general-21'] ?></a>
                        <?php if ($user->cdp_hasPermission('financial_sheet')) { ?>
                        <a href="financial_sheet_consolidation.php" class="btn btn-sm btn-outline-dark">Financial Sheet</a>
                        <?php } ?>
                    </div>
                </div>
            </div>
    ...
    <script>window.cdpDashTable = { url: './ajax/dashboard/consolidated/load_consolidated_ajax.php', target: '.outer_div' };</script>
