...
function cdp_load(page) {
	...
	$("#loader").fadeIn('slow');
	$.ajax({
		url: './ajax/tools/permissions/asingrole_list_ajax.php',
		data: parametros,
    ...
    swal({
        ...
        preConfirm: function() {
            return new Promise(function(resolve) {
                $.ajax({
                        url: './ajax/tools/permissions/asingrole_delete_ajax.php',
                        type: 'POST',
                        data: {
                            'id': role_id,
                        },
...
$(document).ready(function() {
    $("#save_data").submit(function(event) {
       ...
       // Almacena los nombres de los campos faltantes
        ...
        $.ajax({
            url: "ajax/tools/permissions/asingrole_add_ajax.php",
            type: 'POST',
            data: data,
            contentType: false,
            cache: false,
...
$(document).ready(function() {
    ...
    $("#update_data").submit(function(event) {
        ...
        // Validar campos de cdb_user_module_actions (action_name, description_module, action_abbreviation)
        $('input[name^="action_name"], input[name^="description_module"]').each(function() {
            if ($(this).val() === '') {
                camposVacios.push($(this).attr('id') || $(this).attr('name'));  // Verifica por ID o nombre si no tiene ID
            }
        });
        ...
        var data = new FormData(this);
        $.ajax({
            url: "./ajax/tools/permissions/asingrole_edit_ajax.php",
            type: 'POST',
            data: data,
            contentType: false,
            cache: false,
            ...
            success: function(response) {
                Swal.close();
                console.log(response); // Verifica la respuesta en la consola para depuración
                if (response.status === 'success') {
                    Swal.fire({
                        type: 'success',
                        title: message_error_form15,
                        ...
                        window.location.href = 'asingrole_list.php';
                    });
                } else {
                    Swal.fire({
                        type: 'error',
