...
function cdp_load(page, range, table_page) {
    ...
    $.ajax({
        url: './ajax/reports/report_top_users_ajax_air.php',
        data: parametros,
...
function cdp_exportExcel(page) {
	...
	window.open('top_users_air_excel.php?range=' + daterange + '&page=' + page);
}
...
function cdp_exportPrint(page) {
	...
	window.open('top_users_air_print.php?range=' + daterange + '&page=' + page);
}
