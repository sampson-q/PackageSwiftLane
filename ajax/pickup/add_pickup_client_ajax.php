...
if (empty($errors)) {
    ...
    // Uniqueness: reserve the number server-side (see helpers/unique_ids.php).
    $next_order = cdp_uidClaim('order_no');
    ...
    if ($shipment_id !== null) {
        ...
        $pcli_recipient_type = cdp_sanitize($_POST['recipient_type'] ?? 'recipient');
        $pcli_recip_addr = ($pcli_recipient_type === 'user')
        ...
        $dataAddresses = array(
            ...
            'recipient_country'  => $pcli_recip_addr ? cdp_resolveAddressName($pcli_recip_addr->country ?? '', 'cdp_getCountry', $pcli_recip_addr->legacy_country ?? '') : '',
            'recipient_state'    => $pcli_recip_addr ? cdp_resolveAddressName($pcli_recip_addr->state   ?? '', 'cdp_getState',   $pcli_recip_addr->legacy_state   ?? '') : '',
            'recipient_city'     => $pcli_recip_addr ? cdp_resolveAddressName($pcli_recip_addr->city    ?? '', 'cdp_getCity',    $pcli_recip_addr->legacy_city    ?? '') : '',
            'recipient_zip_code' => $pcli_recip_addr ? ($pcli_recip_addr->zip_code ?? '') : '',
            'recipient_address'  => $pcli_recip_addr ? ($pcli_recip_addr->address  ?? '') : '',
        );
