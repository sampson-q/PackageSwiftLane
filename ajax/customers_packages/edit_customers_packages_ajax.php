...
if (empty($errors)) {
    ...
    if ($old_shipment) {
        ...
        $db_old->cdp_query("SELECT courier_name FROM cdb_courier_com WHERE id = :id LIMIT 1");
        $db_old->bind(':id', (int)$old_shipment->order_courier);
        $db_old->cdp_execute();
        $r = $db_old->cdp_registro();
        $old_courier_name = $r ? $r->courier_name : '';

        $db_old->cdp_query("SELECT s_name FROM cdb_shipping_mode WHERE id = :id LIMIT 1");
        $db_old->bind(':id', (int)$old_shipment->order_service_options);
        $db_old->cdp_execute();
        $r = $db_old->cdp_registro();
        $old_service_type = $r ? $r->s_name : '';

        $db_old->cdp_query("SELECT deli_time_description FROM cdb_delivery_time WHERE id = :id LIMIT 1");
        $db_old->bind(':id', (int)$old_shipment->order_deli_time);
        $db_old->cdp_execute();
        $r = $db_old->cdp_registro();
        $old_delivery_time = $r ? $r->deli_time_description : '';
