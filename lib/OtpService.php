...
class OtpService {
    ...
    public function createChallenge($userId, $purpose, array $metadata = [], $ttlSeconds = 300) {
        ...
        // ── Hard cap: max 5 OTPs per user+purpose in any 30-minute window ────────
        $this->db->cdp_query("SELECT COUNT(*) AS cnt FROM cdb_auth_otp_challenges
            WHERE user_id    = :user_id
              AND purpose    = :purpose
              AND created_at >= :window_start");
        $this->db->bind(':user_id',      (int) $userId);
        $this->db->bind(':purpose',      $purpose);
        $this->db->bind(':window_start', date('Y-m-d H:i:s', time() - 1800));
        $countRow = $this->db->cdp_registro();

        if ($countRow && (int) $countRow->cnt >= 5) {
            return [
                'id'       => 0,
                'code'     => null,
                'blocked'  => true,
    ...
    public function verifyChallenge($challengeId, $code, $purpose) {
        ...
        // FIX: check BEFORE incrementing so the 5th wrong attempt correctly locks
        // rather than giving one free extra guess
        ...
        if (!hash_equals($row->code_hash, $expected)) {
            // Increment first, then check if this attempt just hit the cap
            $this->db->cdp_query("UPDATE cdb_auth_otp_challenges
            ...
            $remaining = (int) $row->max_attempts - $newAttempts;
            return [
                'ok'    => false,
    ...
    /**
     * Returns the current challenge row for a given challenge ID.
     * Used by auth-otp.php to pass status + created_at to the JS timer.
     */
    public function getChallengeInfo($challengeId) {
        if ($challengeId <= 0) return null;
        $this->db->cdp_query("SELECT id, status, created_at FROM cdb_auth_otp_challenges
            WHERE id = :id LIMIT 1");
        $this->db->bind(':id', (int) $challengeId);
